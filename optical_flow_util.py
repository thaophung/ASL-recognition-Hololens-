...
#    # create directory for val data
    dest_val_dir = os.path.join(dest_dir, 'val')
#    if os.path.exists(dest_val_dir):
#        print(dest_val_dir, ' already exists')
#    else:
#        os.mkdir(dest_val_dir)
#        print(dest_val_dir, 'created')
#
    dir_mapping = OrderedDict(
           # [(train_dir, dest_train_dir), 
           [(val_dir, dest_val_dir)]) # the mapping between source and dest
...
def _calc_optical_flow(prev,next_):
    flow = cv2.calcOpticalFlowFarneback(prev, next_, flow=None, pyr_scale = 0.5, levels=3,
            winsize=15, iterations=3, poly_n=5, poly_sigma=1.2, flags=0)
    return flow

if __name__ == '__main__':
    src_dir = '/Users/thaophung/workspace/senior_design/dataset/npy_dataset_1'
    dest_dir = '/Users/thaophung/workspace/senior_design/dataset/optical_flow_dataset_1'
