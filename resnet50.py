...
from keras.layers import GlobalMaxPooling2D, ZeroPadding2D, AveragePooling2D
...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2,2)):
    ...
    # Arguments
        input_tensor: input tensor
        kernel_size = default 3, the kernel size of middel conv layer at main path 
        filters: list of integers, the filters of 3 conv layer at main path 
        stage: integer, current stage label, used for generating layer names
        block: 'a', 'b'..., current block label, used for generating layer names
    ...
    Note that froms tage 3, the first conv layer at main path is with strides=(2,2)
...
def ResNet50(include_top=True, weights='imagenet', input_tensor=None, 
    ...
    The model and the weights are compatible with both TensorFlow an Theano.
    The data format convention used by the model is the one specified in
    ...
    # Arguments
        ...
        classes: optional number of classes to classify images into, 
            only to be specified if 'include_top' is True, and if no 
    ...
    if weights not in {'imagenet', None}:
        raise ValueError('The weights argument should be either '
                'None (random initilization) or imagenet (pre-training on ImageNet).')
    ...
    input_shape = _obtain_input_shape(input_shape, default_size=224,
            min_size=197, data_format=K.image_data_format(),
